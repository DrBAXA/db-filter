...
import com.t360.query.parsing.QueryParser;
import com.t360.tables.MidMatchStrategy;
...
class IntegrationTest {
	...
	void test_simpleRequest() throws JsonProcessingException, SQLException {
		...
		final FilteringNode<NegotiationRow> queryNode = queryParser.parse(jsonQuery, Negotiation.class);

		final List<NegotiationRow> all = query(GET_NEGOTIATIONS_TEMPLATE, any -> {
		} , IntegrationTest::parseNegotiation);
	...
	void test_simpleAndOperator() throws JsonProcessingException, SQLException {
		...
		String whereClause = StringUtils.isBlank(queryNode.asSqlWhereClause()) ? "" : " WHERE " + queryNode.asSqlWhereClause();
		String orderByClause = StringUtils.isBlank(queryNode.asSqlOrderByClause()) ? "" : " ORDER BY " + queryNode.asSqlOrderByClause();

		final String withSQLPredicate = GET_NEGOTIATIONS_TEMPLATE + whereClause + orderByClause;
	...
	void test_complex() throws JsonProcessingException, SQLException {
		...
		String json = "{\"expression\":\"A&(B|C)\",\"predicates\":{\"A\":{\"field\":\"Currency1\",\"value\":\"USD\",\"operator\":\"=\"},\"B\":{\"field\":\"AggressiveCompany\",\"value\":10030,\"operator\":\">\"},\"C\":{\"field\":\"PassiveCompany\",\"value\":[232929,232928,232927,232926],\"operator\":\"IN\"}},\"sorting\":[{\"field\":\"Currency1\",\"direction\":\"ASC\"},{\"field\":\"AggressiveCompany\",\"direction\":\"DESC\"}]}";
		final JsonQuery jsonQuery = parseJson(json);
		...
		final List<NegotiationRow> filteredByPredicate = all.stream()
				.filter(queryNode.generateJavaPredicate())
				.sorted(queryNode.generateComparator())
	...
	void test_complex2() throws JsonProcessingException, SQLException {
		...
		final List<MidMatchStrategyRow> all = query(GET_MID_MATCH_STRATEGY_TEMPLATE, any -> {
		} , IntegrationTest::parseMidMatchStrategy);
