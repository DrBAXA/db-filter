...
public class JsonToSqlParserApp {
	...
	private static <T, F extends Enum<F> & ColumnDescription<T>> void executeJsonQuery(String testName, String jsonInput, Class<F> tableEnum)
			throws SQLException {
		QueryTreeParsingService parsingService = new QueryParser();
		final JsonQuery jsonQuery = JsonParsingUtil.parseJson(jsonInput);

		FilteringNode<T> rootNode = parsingService.parse(jsonQuery, tableEnum);
	...
	private static <T extends Enum<T>> String resolveTableName(Class<T> table) {
		...
		if (table.equals(Negotiation.class))
			return "NEGOTIATION";
		if (table.equals(MidMatchStrategy.class))
			return "MID_MATCH_STRATEGY";
